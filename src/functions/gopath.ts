...
export async function createNextTo(source: WebextTab, url: string): Promise<WebextTab> {
  ...
  if (webext.env.isFirefox && source.cookieStoreId) {
    (props as unknown as { cookieStoreId: string }).cookieStoreId = source.cookieStoreId;
  }
  return webext.tabs.create(props);
}

/** Chrome-only: add created tab to same group as source (if any). */
export async function groupIntoSameAs(source: WebextTab, created: WebextTab): Promise<void> {
...
export async function groupIntoNamed(source: WebextTab, created: WebextTab, title: string): Promise<void> {
  if (!webext.env.isChrome) return;
  if (created.id === undefined) return;
